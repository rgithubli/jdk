...
class MallocMemorySummary : AllStatic {
 ...
 public:
  ...
  static bool total_limit_reached(size_t s, size_t so_far, const nmMemlimit* limit);

  // Called when a total limit break was detected.
  // Will return true if the limit was handled, false if it was ignored.
  static bool category_limit_reached(MemTag mem_tag, size_t s, size_t so_far, const nmMemlimit* limit);
