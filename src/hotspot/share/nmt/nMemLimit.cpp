...
NMemLimitSet NMemLimitHandler::_limits;
bool NMemLimitHandler::_have_limit = false;
...
void NMemLimitSet::print_on(outputStream* st) const { // TODO: need to mmap
...
#define BAIL_UNLESS(condition, errormessage) if (!(condition)) { *err = errormessage; return false; }
  ...
  // Category-specific form?
    ...
    while (!sst.eof()) {
      ...
      nmMemlimit* const modified_limit = &_cat[NMTUtil::tag_to_index(mem_tag)];
...
void NMemLimitHandler::initialize(const char* options) {
  _have_limit = false;
...
const char* NMemLimitHandler::NMemTypeToString(NMemType type) {
  switch (type) {
